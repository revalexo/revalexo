#!/bin/bash
# =============================================================================
# 5-seed training: Healthy -> Sarcopenic cross-modality benchmark.
#
# Trains the IMU-only student on the 7 healthy older adults (video available
# only during training) and tests on the 10 sarcopenic subjects baked into the
# sarcopenic config's test_subjects. One run per seed; checkpoints land under
# OUTPUT_BASE/<config>/seed_<seed>/ for the eval scripts to consume.
#
# Usage:
#   bash train_sarcopenic_5seed.sh                 # all configs, 5 seeds each
#   bash train_sarcopenic_5seed.sh <config_name>   # a single config, 5 seeds
# Override GPU:
#   CUDA_VISIBLE_DEVICES=0 bash train_sarcopenic_5seed.sh
# Override repo location (auto-detected from this script otherwise):
#   REPO_DIR=/path/to/revalexo/train bash train_sarcopenic_5seed.sh
# =============================================================================

set -e

# Repo root is three levels up: hpc_scripts/cross_modality/train_sarcopenic/<script>.
...
LOG_DIR="${REPO_DIR}/logs/train_sarcopenic_5seed"
...
for config in "${CONFIGS[@]}"; do
    for seed in "${SEEDS[@]}"; do
        ...
        echo "============================================"
        echo "[TRAIN-SARCO] config=${config} seed=${seed}"
        ...
        if [ ${exit_code} -ne 0 ]; then
            echo "[TRAIN-SARCO] FAILED: ${config} seed=${seed} (exit ${exit_code})" \
                | tee -a "${LOG_DIR}/summary.log"
        else
            echo "[TRAIN-SARCO] DONE: ${config} seed=${seed}" \
                | tee -a "${LOG_DIR}/summary.log"
        fi
...
echo "[TRAIN-SARCO] All runs complete. See ${LOG_DIR}/summary.log"
