...
if args.pretrain_path != 'None' and args.load_prepretrain:
    ...
    for key in mdl_weight.keys():
        if 'graph' not in key:
            # Check if shapes match (skip keys with size mismatch)
            if key in model_state_dict and mdl_weight[key].shape != model_state_dict[key].shape:
                skipped_keys.append(f"{key}: checkpoint {mdl_weight[key].shape} vs model {model_state_dict[key].shape}")
                continue
            useful_weight[key] = mdl_weight[key]

    if skipped_keys:
        print(f"Skipping {len(skipped_keys)} keys with shape mismatch (due to different imu_channel_num):")
