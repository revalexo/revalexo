...
class MultimodalSensorDataset(Dataset):
    ...
    def __init__(self, 
                 ...
                 debug_mode=False,
                 transition_window_size=0.5,  # DEPRECATED (Using labelled transition windows)
                 prediction_horizons=[0],
                 eval_stride=0.25,  # Stride for evaluation sampling
                 clip_stride=8.0,  # Non-overlapping portion between clips
                 base_seed=42):
        """
        ...
        # Exclude background samples if specified
    ...
    def _validate_prediction_horizons(self):
        """Validate prediction horizons against clip duration with enhanced checks for evaluation."""
    ...
    def _filter_short_samples(self, min_duration=10.0):
        """Remove samples shorter than min_duration seconds."""
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            tuple: Data for each requested modality, plus label indices for all horizons, plus transition flags list
        """
        # Use the appropriate sample selection method based on split
        if self.split in ['val', 'test']:
